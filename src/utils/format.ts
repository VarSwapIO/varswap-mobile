...
export function formatTokenDecimals(amount: number, decimals = 18) {
  let amountD = amount / Math.pow(10, decimals);
  return Math.floor(amountD * Math.pow(10, decimals)) / Math.pow(10, decimals);
}

export const percentBetweenTwoValue = (oldValue: string, newValue: string) => {
  const old_value = parseInt(oldValue || "0");
  const new_value = parseInt(newValue || "0");
  ...
  if (!new_value) {
    return { value: "-100%", increase: false };
  }
  const value = (((new_value - old_value) / old_value) * 100).toFixed(2);
  if (new_value >= old_value) {
    return { value: `+${value}%`, increase: true };
  }
  return { value: `${value}%`, increase: false };
};
