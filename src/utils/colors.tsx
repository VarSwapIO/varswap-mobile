...
import { theme as FixedTheme, Theme } from "styles/theme";
import { ChainId } from "constants/chains";
import { assert } from "utils/validation";
import { GlobalPalette } from "styles/color";
import { colors as GlobalColors } from "styles/color";
...
export function useExtractedColors(
  ...
  useEffect(() => {
    ...
    ImageColors.getColors(imageUrl, {
      ...
      const { background, detail, secondary, primary } =
        result as IOSImageColors;
      setColors({
        background,
        detail,
        secondary,
 ...
 */
function pickContrastPassingColor(
...
export function findNearestThemeColor(
  hexString: string
): keyof GlobalPalette | undefined {
  return Object.keys(GlobalColors).reduce(
    (closestMatch, currentColorName) => {
      ...
      if (
        colorDiff &&
        (!closestMatch.colorDiff || colorDiff < closestMatch.colorDiff)
      ) {
        return {
          colorDiff,
          colorName: currentColorName as keyof GlobalPalette,
        };
      }
      return closestMatch;
    },
...
export const actionLabelMyActivitiesShort = (status: string) => {
  switch (status) {
    case "LIST":
      return `Listed`;
    case "SALE":
...
export const getColorActivity = (status: string) => {
  switch (status) {
    case "LISTED":
      return `#3A82F6`;
    case "BOUGHT":
