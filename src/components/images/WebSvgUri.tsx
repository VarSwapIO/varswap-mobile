...
import { fetchSVG } from "components/images/utils";
...
export function WebSvgUri({
  autoplay,
  maxHeight,
  uri,
}: SvgUriProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function fetchSvg(): Promise<void> {
      try {
        const { content, aspectRatio: _aspectRatio } = await fetchSVG(
          uri,
          autoplay,
          signal
        );
        ...
        logger.error(
          "SvgUri",
          "fetchSvg",
          "Failed to fetch remote SVG content",
          err
        );
      }
    }
