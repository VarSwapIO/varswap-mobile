...
import { AnimatedBox, Flex } from "components/layout";
...
export function NotificationToast({
  ...
  useEffect(() => {
    if (currentNotification) {
      bannerOffset.value = withDelay(
        100,
        withSpring(showOffset, SPRING_ANIMATION)
      );
    }
  ...
  const dismissLatest = useCallback(() => {
    ...
    setTimeout(
      () => dispatch(notificationActions.popNotification({ address })),
      500
    );
  ...
  return (
    ...
    >
      <AnimatedBox
        borderColor={useSmallDisplay ? "none" : "outline"}
        ...
        {actionButton && (
          <Flex shrink alignItems="flex-end" flexBasis="25%" gap="spacing4">
            <TouchableArea p="spacing8" onPress={actionButton.onPress}>
              <Text color="primary">{actionButton.title}</Text>
            </TouchableArea>
          </Flex>
        )}
      </Flex>
  ...
  return (
    <Flex row flexShrink={1} justifyContent="center" pointerEvents="box-none">
      ...
      >
        ...
        >
          {icon}
          <Text adjustsFontSizeToFit numberOfLines={1} variant="largeBold">
            {title}
          </Text>
        </Flex>
      </TouchableArea>
